...
while getopts "f" opt; do
    case $opt in
        f) FORCE_MODE="true" ;;
        *) echo "Usage: install_pi.sh [-f]" >&2; exit 1 ;;
    esac
done
...
if [ ! -d "$AGENTS_DIR" ]; then
    log_error "✗ Error: Agents directory not found: $AGENTS_DIR"
    exit 2
fi

if [ ! -d "$PI_EXTENSION_SOURCE" ]; then
    log_error "✗ Error: Task tool extension directory not found: $PI_EXTENSION_SOURCE"
    exit 2
fi
